...
server <- function(input, output) {
  ...
  output$daily_bar <- renderPlot({
    ggplot(ddata())+
    geom_line(aes(x= Day, y=High), colour = "red", size=1) +
    geom_line(aes(x= Day, y=Low), colour = "blue",size = 1) + 
      labs(x = "Days", y="Price") +
      scale_color_manual(name = "Price", values = c("High Price" = "red", "Low Price" = "blue"))+
